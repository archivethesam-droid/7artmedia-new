...
async function build() {
  try {
    ...
    if (error.code === 'ENOENT') {
      console.warn(`Build warning: ${CONTENT_DIR} is missing.`);
  ...
  if (!contentFiles.some((file) => file.endsWith('.md'))) {
    console.warn(`Build warning: ${CONTENT_DIR} contains no Markdown posts.`);
    console.warn('Keeping the existing prebuilt blog files and continuing with export.');
    return;
  }
